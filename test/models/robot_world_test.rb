...
class RobotWorldTest < Minitest::Test
  ...
  def test_we_can_create_a_robot
    ...
    assert_equal first_robot_id, robot.id
  ...
  def test_we_can_find_a_robot_by_its_id
    ...
    robot = RobotWorld.find((first_robot_id)+2)
    assert_equal "Wall-E - version: 2", robot.name
  end

  def test_we_can_update_an_existing_robot
    create_robots(1)

    robot = RobotWorld.find(first_robot_id)
    assert_equal "Wall-E - version: 0", robot.name

    RobotWorld.update(first_robot_id, {:name => "Wall-E (updated)"})
    updated_robot = RobotWorld.find(first_robot_id)
  ...
  def test_we_can_delete_a_robot
    ...
    assert_equal 5, RobotWorld.all.size
    RobotWorld.delete(first_robot_id)
    assert_equal 4, RobotWorld.all.size
    RobotWorld.delete((first_robot_id + 2))
